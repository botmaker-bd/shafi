...
// Update command cache function
async function updateCommandCache(token) {
    try {
        console.log(`🔄 Updating command cache for bot: ${token}`);
        
        // Get latest commands from database
        const { data: commands } = await supabase
            .from('commands')
            .select('*')
            .eq('bot_token', token)
        ...
        if (commands) {
            ...
            botCommands.set(token, commands);
            console.log(`✅ Command cache updated with ${commands.length} commands`);
        }
...
// Update the add command function in router
// In the add bot route - FIX WEBHOOK URL
router.post('/add', async (req, res) => {
    try {
        ...
        // ✅ CORRECT WEBHOOK URL
        const webhookUrl = `https://bot-maker-jcch.onrender.com/bot/${token}`;
        ...
        const { data: botData, error } = await supabase
            ...
            .insert([{
                ...
                user_id: userId,
                webhook_url: webhookUrl, // ✅ Use correct URL
                is_active: true
            }])
            .select()
            .single();
        ...
        await updateCommandCache(token);
...
// Add this to exports at the end of bot-manager.js
module.exports = {
    router,
    handleBotUpdate,
    updateCommandCache, // ✅ Add this export
    initializeBot // ✅ Add this export if needed
};