// server/core/python-runner.js - MULTI-PLATFORM COMPATIBLE VERSION
...
class PythonRunner {
    ...
    getTempDir() {
        if (this.isVercel) {
            // Vercel-এ /tmp ব্যবহার করুন (read/write access আছে)
            return '/tmp/python-temp';
        } else {
            // Local বা Render-এ regular temp directory
            return path.join(__dirname, '../../temp');
        }
    }

    findPythonCommand() {
        // Vercel-এ Python নেই, তাই error throw করবে না
        if (this.isVercel) {
            return 'python3'; // Placeholder,但实际上不会执行
        }
    ...
    async initialize() {
        ...
        // Vercel-এ Python initialization skip করুন
        if (this.isVercel) {
    ...
    async checkPython() {
        if (this.isVercel) {
            throw new Error('Python execution is not supported on Vercel platform');
        }
    ...
    runPythonCodeSync(code) {
        // Vercel-এ Python execution block করুন
        if (this.isVercel) {
            throw new Error('Python code execution is not available on Vercel. Please use Render or other platforms that support Python.');
        }
    ...
    runPythonFile(code) {
        if (this.isVercel) {
            throw new Error('Python file execution not supported on Vercel');
        }
...
${this.indentCode(code)}
    ...
    getPlatformInfo() {
        return {
            platform: this.isVercel ? 'Vercel' : 'Other',
            pythonSupported: !this.isVercel,
    ...
    async installPythonLibrary(libraryName) {
        if (this.isVercel) {
            throw new Error('Python library installation is not supported on Vercel');
        }
    ...
    async getInstalledLibraries() {
        if (this.isVercel) {
            return []; // Vercel-এ কোনো installed libraries নেই
        }
    ...
    async uninstallPythonLibrary(libraryName) {
        if (this.isVercel) {
            throw new Error('Python library uninstallation is not supported on Vercel');
        }
